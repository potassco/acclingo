...
class SMAC(object):
    ...
    def __init__(self,
        ...
        if intensifier is None:
            ...
            #TODO: Hack -- should be done in SMAC
            intensifier.stats = self.stats
            intensifier.traj_logger = traj_logger
            # general attributes
            inten_instances = scenario.train_insts
            if inten_instances is None:
                inten_instances = []
            intensifier.instances = set(inten_instances)
            intensifier.maxR = scenario.maxR
            intensifier.minR = scenario.minR
            intensifier.rs = rng
    
            # scenario info
            intensifier.cutoff = scenario.cutoff
            intensifier.deterministic = scenario.deterministic
            intensifier.run_obj_time = scenario.run_obj == "runtime"
